 ...
 * Instructor-only: add lecture with an order
 */
export const addLecture = async (req, res) => {
  ...
  try {
    ...
    // If it's a unique index error on order
    if (error.code === 11000) {
 ...
 * - If instructor requests -> return full lectures
 * - If student -> return sanitized lectures (no correctAnswer)
 */
 ...
 * Student gating: ensure previous lectures (by order) are completed
 */
export const getLectureById = async (req, res) => {
  ...
  try {
    ...
    // If user is instructor and owner -> full lecture
    if (req.user && req.user.role === "instructor" && req.user._id.toString() === String(lecture.course.instructor)) {
      return res.json(lecture);
    }

    // Student gating: if authenticated student, check progress
    if (req.user && req.user.role === "student") {
      // load all lectures for this course sorted by order
      ...
      // find the requested lecture's order
      ...
      // ensure all prior orders are completed
    ...
    // If unauthenticated or other roles: return sanitized (but gating enforced for unauth too)
    // To access course lectures, user normally should enroll -> but we enforce gating only for students.
    return res.json(sanitizeLectureForStudent(lecture));
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
